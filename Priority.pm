...
sub highest_first {
	...
	my $e_old;
	while(defined($e_old = $self->{'.heap'}->extract_minimum)) {
	    my $e_new = Heap::Elem::NumRev->new;
	    $e_new->val($e_old->val);
	    $n->add($e_new);
	}
	$self->{'.heap'} = $n;	    
...
sub get_priority_levels {
    my $self = shift;
    my @levels = $self->{'.hifirst'} ? sort {$b<=>$a}  keys %{$self->{'.queues'}} :
	                               sort {$a<=>$b}  keys %{$self->{'.queues'}} ;
...
sub get_level {
	...
	$self->_error("Priority must be a decimal number!\n");
	return;
    }
...
sub pop {
	...
	my $garbage = $self->{'.heap'}->extract_minimum;
    }
    $self->{'.count'}--;
    return $retval;
}
...
sub delete_item {
	...
	@levels = $self->get_priority_levels;
    }
...
sub delete_priority_level {
    ...
    my $i;
    while(defined($i = $self->{'.heap'}->extract_minimum)) {
	last if ($i->val eq $priority);
	$tmp->add($i);
    }
